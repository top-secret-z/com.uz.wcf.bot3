...
class NotifyScheduler {
	...
	public function schedule(array $data) {
		...
		foreach ($botContents as $key => $botContent) {
			// language
			if ($key == 0) $languages[$key] = $defaultLanguage;
			else $languages[$key] = LanguageFactory::getInstance()->getLanguage($key);
			if (!$languages[$key]->languageID) $languages[$key] = $defaultLanguage;
		...
		if ($bot->typeDes == 'system_report') {
			if (empty($affectedUserIDs)) $type->hasAffected = 0;
		}
		...
		if (empty($affectedUserIDs) && $type->allowGuest) $type->hasAffected = 0;
		...
		if (!$notify->hasReceiver) {
			...
			//	$notifyCount = 0;
				...
				if ($bot->notifyLanguageID == -1 && $notify->notifyTitle == 'article') {
					...
					foreach ($languages as $languageID => $language) {
						$send = 0;
						if ($bot->notifyLanguageID == 0 && $languageID == 0) $send = 1;
						if ($bot->notifyLanguageID > 0 && $languageID == $bot->notifyLanguageID) $send = 1;
						if ($bot->notifyLanguageID == -1 && $languageID > 0) $send = 1;
				...
				//	$notifyCount = 0;
					...
					foreach ($languages as $key => $language) {
						...
						foreach ($affectedUserIDs as $userID) {
							...
							if (isset($countToUserID[$userID])) $placeholders['user-count'] = $countToUserID[$userID];
			...
			if (!$type->hasAffected) {
				...
				if (count($receiverIDs)) {
					...
					if ($notify->notifyTitle == 'conversation' && $bot->conversationType == 1) {
						...
						foreach ($languages as $languageID => $language) {
							$send = 0;
							if ($bot->notifyLanguageID == 0 && $languageID == $defaultLanguage->languageID) $send = 1;
							elseif ($bot->notifyLanguageID > 0 && $languageID == $bot->notifyLanguageID) $send = 1;
							elseif ($bot->notifyLanguageID == -1 && $languageID > 0) $send = 1;
						...
						foreach ($receiverIDs as $receiverID) {
							...
							foreach ($languages as $languageID => $language) {
								$send = 0;
								if ($bot->notifyLanguageID == 0 && $languageID == $receiverLanguageID) $send = 1;
								elseif ($bot->notifyLanguageID > 0 && $languageID == $bot->notifyLanguageID) $send = 1;
								elseif ($bot->notifyLanguageID == -1 && $languageID > 0) $send = 1;
