...
class UzbotFeedreaderCronjob extends AbstractCronjob
{
    ...
    public function execute(Cronjob $cronjob)
    {
        ...
        foreach ($bots as $bot) {
            ...
            if (!$bot->testMode) {
                if ($bot->feedreaderLast + $bot->feedreaderFrequency > TIME_NOW) {
                    continue;
                }
            ...
            $positiveFilterWords = [];
            $negativeFilterWords = [];
            if ($bot->feedreaderInclude) {
                $positiveFilterWords = \array_unique(ArrayUtil::trim(\explode(',', \mb_strtolower($bot->feedreaderInclude))));
            }
            if ($bot->feedreaderExclude) {
                $negativeFilterWords = \array_unique(ArrayUtil::trim(\explode(',', \mb_strtolower($bot->feedreaderExclude))));
            }
            ...
            if ($type == 'rss') {
                ...
                if ($channel->getElementsByTagName('title')->length != 0) {
                    $feedData['feed-title'] = $channel->getElementsByTagName('title')->item(0)->nodeValue;
                }
                if ($channel->getElementsByTagName('link')->length != 0) {
                    $feedData['feed-link'] = $channel->getElementsByTagName('link')->item(0)->nodeValue;
                }
                if ($channel->getElementsByTagName('description')->length != 0) {
                    $feedData['feed-description'] = $channel->getElementsByTagName('description')->item(0)->nodeValue;
                }
                ...
                foreach ($items as $item) {
                    // check for max items
                    if ($bot->feedreaderMaxItems && $count >= $bot->feedreaderMaxItems) {
                        break;
                    }
                    $count++;
                    ...
                    if (isset($data['pubDate'])) {
                        ...
                        if ($bot->feedreaderMaxAge) {
                            if ($time < TIME_NOW - $bot->feedreaderMaxAge * 86400) {
                                continue;
                            }
                        }
                    } else {
                    ...
                    if (empty($data['content:encoded'])) {
                        $data['content:encoded'] = $data['description'];
                    }
                    ...
                    if (!empty($data['guid'])) {
                        ...
                        if (isset($hashData[$hash2]) || isset($hashData[$hash])) {
                            continue;
                        }
                    } else {
                        $hash = \sha1($data['title'] . $data['description']);
                        if (isset($hashData[$hash])) {
                            continue;
                        }
                    }
            ...
            if ($type == 'feed') {
                ...
                if ($feed->getElementsByTagName('id')->length != 0) {
                    $id = $feed->getElementsByTagName('id')->item(0)->nodeValue;
                }
                if ($feed->getElementsByTagName('title')->length != 0) {
                    $feedData['feed-title'] = $feed->getElementsByTagName('title')->item(0)->nodeValue;
                }
                ...
                if ($feed->getElementsByTagName('subtitle')->length != 0) {
                    ...
                    $feedData['feed-description'] = $feedData['feed-title'];
                }
            ...
            if (\count($feeditemData)) {
                ...
                \usort($feeditemData, static function ($a, $b) {
                    if (\strtotime($a['feeditem-time']) > \strtotime($b['feeditem-time'])) {
                        return 1;
                    }
                    if (\strtotime($a['feeditem-time']) < \strtotime($b['feeditem-time'])) {
                        return -1;
