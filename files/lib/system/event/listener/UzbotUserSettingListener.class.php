...
class UzbotUserSettingListener implements IParameterizedEventListener
{
    ...
    public function execute($eventObj, $className, $eventName, array &$parameters)
    {
        ...
        if ($className == 'wcf\data\user\UserAction') {
            ...
            if ($eventObj->getActionName() == 'update') {
                ...
                if (isset($params['data']['enableGravatar'])) {
                    if ($params['data']['enableGravatar']) {
                        $change['avatar'] = 'gravatar';
                    } else {
                        if (isset($params['data']['avatarID'])) {
                            $change['avatar'] = 'no';
                        } else {
                            $change['avatar'] = 'yes';
                        }
                ...
                if (isset($params['data']['signature'])) {
                    $change['signature'] = $params['data']['signature'];
                }
                if (isset($params['data']['email'])) {
                    $change['email'] = $params['data']['email'];
                }
                if (isset($params['data']['username'])) {
                    $change['username'] = $params['data']['username'];
                }
                if (isset($params['data']['oldUsername'])) {
                    $change['oldUsername'] = $params['data']['oldUsername'];
                }
                if (isset($params['data']['userTitle']) && isset($objects[0]->userTitle)) {
                    if ($objects[0]->userTitle != $params['data']['userTitle']) {
                        $change['userTitle'] = $params['data']['userTitle'];
                    }
                }
                ...
                if (isset($params['data']['quitStarted']) && $params['data']['quitStarted'] > 0) {
                    $change['quitStarted'] = $params['data']['quitStarted'];
                }
                if (isset($params['data']['quitStarted']) && $params['data']['quitStarted'] == 0) {
                    $change['quitEnded'] = 0;
                }
        ...
        foreach ($bots as $bot) {
            ...
            if ($bot->userSettingAvatarOption && isset($change['avatar'])) {
                $setting['avatar'] = $change['avatar'];
            }
            if ($bot->userSettingSignature && isset($change['signature'])) {
                $setting['signature'] = $change['signature'];
            }
            if ($bot->userSettingEmail && isset($change['email'])) {
                $setting['email'] = $change['email'];
            }
            if ($bot->userSettingUsername && isset($change['username'])) {
                $setting['username'] = $change['username'];
            }
            if ($bot->userSettingUsername && isset($change['oldUsername'])) {
                $setting['oldUsername'] = $change['oldUsername'];
            }
            if ($bot->userSettingUserTitle && isset($change['userTitle'])) {
                $setting['userTitle'] = $change['userTitle'];
            }
            if ($bot->userSettingSelfDeletion && isset($change['quitStarted'])) {
                $setting['quitStarted'] = $change['quitStarted'] + 7 * 24 * 3600;
            }
            if ($bot->userSettingSelfDeletion && isset($change['quitEnded'])) {
                $setting['quitEnded'] = $change['quitEnded'];
            }
            if ($bot->userSettingOther && isset($change['options'])) {
                $setting['options'] = $change['options'];
            }
            if ($bot->userSettingCover && isset($change['cover'])) {
                $setting['cover'] = $change['cover'];
            }
