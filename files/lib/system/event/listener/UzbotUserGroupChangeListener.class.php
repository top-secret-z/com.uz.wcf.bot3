...
class UzbotUserGroupChangeListener implements IParameterizedEventListener
{
    ...
    public function execute($eventObj, $className, $eventName, array &$parameters)
    {
        ...
        if ($action == 'update') {
            ...
            if (!isset($params['groups']) && !isset($params['removeGroups'])) {
                return;
            }
        ...
        foreach ($users as $user) {
            ...
            $beforeAdd = $this->beforeAddIDs[$user->userID] ?? [];
            $beforeRemove = $this->beforeRemoveIDs[$user->userID] ?? [];
            $beforeUpdate = $this->beforeUpdateIDs[$user->userID] ?? [];
