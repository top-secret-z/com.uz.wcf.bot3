...
class UzbotNotifyArticle
{
    ...
    public function send(Uzbot $bot, $text, $subject, $teaser, $language, $receiver, $tags, $imageID, $teaserImageID)
    {
        ...
        if (!$bot->testMode) {
            ...
            if (\mb_strlen($text) > 63500) {
                $text = \mb_substr($text, 0, 63500) . ' ...';
            }
    ...
    public function sendMulti(Uzbot $bot, $contents, $subjects, $teasers, $languageIDs, $receiver, $tags, $imageIDs, $teaserImageIDs)
    {
        ...
        foreach ($languageIDs as $languageID) {
            ...
            if (\mb_strlen($subjects[$languageID]) > 255) {
                $subject = \mb_substr($subjects[$languageID], 0, 250) . '...';
            }
            ...
            if (!$bot->testMode) {
                ...
                $content[$languageID] = [
                    ...
                    'htmlInputProcessor' => $htmlInputProcessors[$languageID] ?? null,
                    'imageID' => $imageIDs[$languageID],
                    'teaserImageID' => $teaserImageIDs[$languageID],
                ];
            } else {
                if (\mb_strlen($contents[$languageID]) > 63500) {
                    $contents[$languageID] = \mb_substr($contents[$languageID], 0, 63500) . ' ...';
                }
