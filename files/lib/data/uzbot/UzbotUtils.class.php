...
class UzbotUtils
{
    ...
    public static function convertPlaceholders($text, $language, array $placeholders = [], $userProfile = null, $replace = false)
    {
        ...
        // period placeholders
        if (isset($placeholders['date-from'])) {
            $placeholders['date-from'] = DateUtil::format(DateUtil::getDateTimeByTimestamp($placeholders['date-from']), DateUtil::DATE_FORMAT, $language);
        }
        if (isset($placeholders['time-from'])) {
            $placeholders['time-from'] = DateUtil::format(DateUtil::getDateTimeByTimestamp($placeholders['time-from']), DateUtil::TIME_FORMAT, $language);
        }
        if (isset($placeholders['date-to'])) {
            $placeholders['date-to'] = DateUtil::format(DateUtil::getDateTimeByTimestamp($placeholders['date-to']), DateUtil::DATE_FORMAT, $language);
        }
        if (isset($placeholders['time-to'])) {
            $placeholders['time-to'] = DateUtil::format(DateUtil::getDateTimeByTimestamp($placeholders['time-to']), DateUtil::TIME_FORMAT, $language);
        }

        // add standard and user placeholders
        if (!isset($placeholders['date'])) {
            $placeholders['date'] = DateUtil::format(DateUtil::getDateTimeByTimestamp(TIME_NOW), DateUtil::DATE_FORMAT, $language);
        }
        if (!isset($placeholders['time'])) {
            $placeholders['time'] = DateUtil::format(DateUtil::getDateTimeByTimestamp(TIME_NOW), DateUtil::TIME_FORMAT, $language);
        }
        if (!isset($placeholders['page'])) {
            $placeholders['page'] = $language->get(PAGE_TITLE);
        }

        if ($replace) {
            // add user specific placeholders, if user is set preset with guest
            if (isset($userProfile)) {
                $placeholders['user-age'] = $userProfile->getAge() ?: 'x';
        ...
        if (isset($placeholders['usergroup'])) {
            $placeholders['usergroup'] = $language->get($placeholders['usergroup']);
        }
        ...
        if (isset($placeholders['feeditem-systemdate'])) {
            $placeholders['feeditem-systemdate'] = DateUtil::format(DateUtil::getDateTimeByTimestamp($placeholders['feeditem-systemdate']), DateUtil::DATE_FORMAT, $language);
        }
        if (isset($placeholders['feeditem-systemtime'])) {
            $placeholders['feeditem-systemtime'] = DateUtil::format(DateUtil::getDateTimeByTimestamp($placeholders['feeditem-systemtime']), DateUtil::TIME_FORMAT, $language);
        }
        ...
        foreach ($placeholders as $key => $placeholder) {
            ...
            if ($key == 'user-count' && !isset($userProfile)) {
                continue;
            }
            if ($key == 'translate') {
                continue;
                ...
                while ($count <= $cirData['cirRepeatCount']) {
                    ...
                    while (1) {
                        ...
                        if ($temp->format('w') == $cirData['cirMonthlyDoW_day']) {
                            $xth++;
                            $match = $temp->format('j');
                            if ($xth == $cirData['cirMonthlyDoW_index']) {
                                break;
                            }
                        }
                ...
                while ($count <= $cirData['cirRepeatCount']) {
                    ...
                    while (1) {
                        ...
                        if ($temp->format('w') == $cirData['cirYearlyDoW_day']) {
                            $xth++;
                            $match = $temp->format('j');
                            if ($xth == $cirData['cirYearlyDoW_index']) {
                                break;
                            }
                        }
                        // add day until next month, reset to actual match
